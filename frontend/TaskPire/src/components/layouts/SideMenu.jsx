...
const SideMenu = ({activeMenu}) => {
    ...
    return <div className="w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20" dir="rtl">
        <div className="flex flex-col items-center justify-center mb-7 pt-5">
            <div className="relative">
                <img src={user?.profileImageURL || ""} className="rounded-full w-20 h-20 bg-slate-400"/>
            </div>
            {user?.role === "admin" && (<div className="text-[10px] font-medium text-white bg-primary px-3 py-0.5 rounded mt-3">ادمین</div>)}
            {user?.role === "user" && (<div className="text-[10px] font-medium text-white bg-primary px-3 py-0.5 rounded mt-3">کاربر عادی</div>)}
